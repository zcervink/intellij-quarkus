...
abstract class GuiTestsTestBase : GuiTestCase(false) {
    protected val ciTimeout = Timeout.timeout(3, TimeUnit.MINUTES)

    companion object {
        private val logger = LoggerFactory.getLogger(GuiTestsTestBase::class.java)
    }
    ...
    fun GuiTestsTestBase.project(body: IdeFrameFixture.() -> Unit) {
        simpleProject {
    ...
    fun IdeFrameFixture.openTestFile() {
        projectView {
            path("SimpleProject", "src", "Main.md").click()
    ...
    fun GuiTestsTestBase.waitADecentMoment() {
